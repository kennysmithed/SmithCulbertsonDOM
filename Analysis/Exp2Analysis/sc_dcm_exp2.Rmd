...
exp2.sov.summary.total.collapsed$Animacy <- "All" 
...
ggplot(data=exp2.sov.summary.by.animacy.collapsed, aes(x=Day, y=isSOV, fill=Animacy,colour=Animacy)) +
  ...
  stat_summary(data=exp2.sov.summary.total.collapsed,geom='errorbar', 
               fun.data='mean_cl_boot',fun.ymin="min", fun.ymax="max",width=0.15,position=position_dodge(0.9),
               color='black',show.legend = FALSE) +
  stat_summary(data=exp2.sov.summary.total.collapsed,geom='point', fun.y='mean', position=position_dodge(0.9),
               size=3) +
  stat_summary(data=exp2.sov.summary.total.collapsed,geom='point', fun.y='mean', position=position_dodge(0.9),
  ...
  scale_colour_manual(values=my.colours,breaks=c("All","FALSE", "TRUE"),labels=c("All data","Inanimate object", "Animate object")) +
  scale_fill_manual(values=my.colours,breaks=c("All","FALSE", "TRUE"),labels=c("All data","Inanimate object", "Animate object")) +
...
#this is aggregating over animacy
exp2.casemarking.summary.total <- aggregate(casemarked~Day+EventType+workerId,data=subset(exp2Data,Block=="SentenceTest1" & grammatical),FUN=mean)
#needs to have this column for plotting compatability with casemarking.summary.by.animacy
exp2.casemarking.summary.total$Animacy <- "All" 
...
ggplot(data=exp2.casemarking.summary.by.animacy, aes(x=Day, y=casemarked,
  ...
  #point 95CI for grand means
  stat_summary(data=exp2.casemarking.summary.total,aes(x=Day,y=casemarked),geom='errorbar',
               fun.data='mean_cl_boot',fun.ymin="min",
               fun.ymax="max",width=0.15,position=position_dodge(0.9),
               colour='black',show.legend = FALSE) +
  stat_summary(data=exp2.casemarking.summary.total,aes(x=Day,y=casemarked),geom='point', fun.y='mean', position=position_dodge(0.9),shape=19,size=3,show.legend = FALSE) +
  stat_summary(data=exp2.casemarking.summary.total,aes(x=Day,y=casemarked),geom='point', fun.y='mean', position=position_dodge(0.9),shape=1,size=3,colour='black',show.legend = FALSE) +
...
#this is aggregating over animacy
exp2.recall.vs.interaction.summary.total <- aggregate(casemarked~Block+EventType+workerId,data=exp2RecallVsInteractionData,FUN=mean)
...
exp2.recall.vs.interaction.summary.total$Block <- revalue(exp2.recall.vs.interaction.summary.total$Block,
                                         c("SentenceTest1"="Recall",
                                           "Interaction"="Interaction"))
exp2.recall.vs.interaction.summary.total$Animacy <- "All" 
```
...
ggplot(data=exp2.recall.vs.interaction.summary, aes(x=Block, y=casemarked, 
  ...
  #point 95CI for grand means
  stat_summary(data=exp2.recall.vs.interaction.summary.total,
                geom='errorbar', fun.data='mean_cl_boot',fun.ymin="min",
                fun.ymax="max",width=0.15,position=position_dodge(0.9),
                color='black',show.legend = FALSE) +
  stat_summary(data=exp2.recall.vs.interaction.summary.total,geom='point', 
